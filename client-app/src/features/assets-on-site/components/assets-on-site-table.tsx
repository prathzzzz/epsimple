...
interface AssetsOnSiteTableProps<TData, TValue> {
  readonly columns: ColumnDef<TData, TValue>[];
}

export function AssetsOnSiteTable<TData, TValue>({
  columns,
}: AssetsOnSiteTableProps<TData, TValue>) {
  const { globalFilter, setGlobalFilter } = useAssetsOnSite();
  ...
  const { data: searchData, isLoading: isSearchLoading } = assetsOnSiteApi.useSearch({
  ...
  const { data: allData, isLoading: isAllLoading } = assetsOnSiteApi.useGetAll({
    page: pagination.pageIndex,
    size: pagination.pageSize,
    sortBy: sorting.length > 0 ? sorting[0].id : 'id',
    sortDirection: sorting.length > 0 && sorting[0].desc ? 'DESC' : 'ASC',
